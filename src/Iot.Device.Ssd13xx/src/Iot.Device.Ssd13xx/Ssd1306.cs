...
namespace IotDevice.Ssd13xx
{
    ...
    public class Ssd1306 : Ssd13xx
    {
        ...
        public void SendCommand(Commands.ISsd1306Command command) => this.SendCommand((Commands.ICommand)command);

        /// <summary>
        /// Sends command to the device.
        /// </summary>
        /// <param name="command">Command being send.</param>
        public override void SendCommand(Commands.ISharedCommand command) => this.SendCommand(command);
        ...
        private void SendCommand(Commands.ICommand command)
        {
            ...
            // Be aware there is a Continuation Bit in the Control byte and can be used to state (logic LOW) if there is only data bytes to follow.
